...
-- Load host-specific configuration (screen roles, etc.)
-- Generated by home-manager in ~/.config/awesome/host.lua
local host_config = {}
do
	local path = os.getenv("HOME") .. "/.awesome-screen-config.lua"
	local f = io.open(path)
	if f then
		f:close()
		host_config = dofile(path)
	end
end
...
awful.screen.connect_for_each_screen(function(s)
	-- Mark screen with its role from host config (e.g. "primary", "secondary")
	if host_config.screen_roles then
		for output_name, role in pairs(host_config.screen_roles) do
			if s.outputs[output_name] then
				s.role = role
			end
		end
	end
...
function open_or_join(instance, program)
	return function(c)
		...
		function find_candidate(instance)
			...
			-- role is set in ~/.awesome-screen-config.lua (via home-manager)
			local role_priority = { primary = 1, secondary = 2, virtual = 3 }
			function screen_priority(c)
				if c.screen == focused_screen then
					return 0
				else
					return role_priority[c.screen.role] or 99
				end
			end

			function compare_candidates(c1, c2)
				local p1 = screen_priority(c1)
				local p2 = screen_priority(c2)
				if p1 ~= p2 then
					return p1 < p2
				end
