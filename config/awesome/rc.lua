...
traybar:setup({ layout = wibox.layout.fixed.horizontal, tray })
...
function open_or_join(instance, program)
	return function(c)
		...
		local result = nil
		if client.focus and client.focus.instance == instance then
...
function tell_pulse_volume()
	awful.spawn.easy_async("clk volume show", function(stdout, stderr, exitreason, exitcode)
		naughty.notify({
...
function tell_brightness()
	awful.spawn.easy_async("clk brightness show", function(stdout, stderr, exitreason, exitcode)
		naughty.notify({
			preset = naughty.config.presets.normal,
			title = "Brightness",
			text = stdout,
...
globalkeys = gears.table.join(
	...
	awful.key({ modkey, "Mod1" }, "Down", function()
		...
		awful.spawn("clk volume down")
		tell_pulse_volume()
	end, { description = "change volume", group = "client" }),
	...
	end, { description = "Modify the current content in emacs", group = "emacs" }),
	...
	awful.key({ modkey, "Mod1" }, "i", function()
		...
		description = "Captured OCR-ed content from printed screen",
		group = "screen",
	}),
	...
	awful.key({ modkey, "Control" }, "i", open_or_join("Navigator", "firefox"), {}),
	awful.key({ modkey, "Control" }, "f", open_or_join("Navigator", "firefox"), {}),
	...
	awful.key({ modkey, "Control" }, "t", open_or_join(terminal_instance, terminal), {
		description = "open a terminal",
		group = "launcher",
	}),
	...
	awful.key({ modkey }, "p", function()
		...
		awful.spawn.easy_async("clk mpc toggle", function(stdout, stderr, exitreason, exitcode)
			naughty.notify({
				preset = naughty.config.presets.normal,
				title = "MPD",
				text = stdout,
	...
	end, { description = "toggle play", group = "music" }),
	...
	awful.key({ modkey, "Shift" }, "p", function()
		awful.spawn.easy_async("mpc", function(stdout, stderr, exitreason, exitcode)
			naughty.notify({
				preset = naughty.config.presets.normal,
				title = "MPD",
				text = stdout,
	...
	awful.key({}, "XF86AudioMute", function()
		awful.spawn.easy_async("clk volume toggle-mute", function(stdout, stderr, exitreason, exitcode)
			tell_pulse_volume()
		end)
	end, { description = "toggle mute", group = "music" }),
	...
	end, { description = "play previous song", group = "music" }),
...
clientkeys = gears.table.join(
	...
	end, { description = "capture a screenshot", group = "org-mode" }),
	awful.key({ modkey }, "F9", function(c)
...
clientbuttons = gears.table.join(
	...
	awful.button({ "Mod1", "Mod4", "Control", "Shift" }, 1, function(c)
		if c.pid then
