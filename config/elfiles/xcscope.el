...
(defcustom cscope-additional-options nil
"additional options to give cscope when calling it
ex : -C for case insensitive
"
  :type '(repeat (string))
  :group 'cscope)
...
(defun cscope-search-one-database ()
  ...
  (let ( next-item options cscope-directory database-file outbuf done
	...
	(when cscope-additional-options
	  (setq options (append cscope-additional-options options))
	  )
