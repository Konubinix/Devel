...
(defun sticky-window-toggle-dedicated ()
  (interactive)
  (let* (
		 (_current-window (selected-window))
		 (_new-status (not (window-dedicated-p _current-window)))
		 )
    (message (concat "This window becomes "
                     (if (not _new-status) "not-")
                     "dedicated") )
    (set-window-dedicated-p _current-window _new-status)
    )
  )
