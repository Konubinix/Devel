...
;; the only hacky thing, set the PLATFORM like python will like it
(setenv "PLATFORM" (cond
  ((or
...
(file-name-directory
  (with-temp-buffer
    (insert-file-contents-literally (expand-file-name (format "~/.env_%s.conf" (getenv "PLATFORM"))))
...
(konix/load-env-file)
(defvar perso-dir (getenv "PERSO_DIR"))
(defvar python-bin (getenv "PYTHON_BIN"))
(defvar config-dir (getenv "CONFIG_DIR") "where I put my config files")
...
""")
    ...
    if not os.path.exists(environ["EMACS_CUSTOM_FILE"]):
        shutil.copyfile("./config/emacs-custom.el", environ ["EMACS_CUSTOM_FILE"])
